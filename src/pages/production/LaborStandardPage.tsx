...
const LaborStandardContent: React.FC = () => {
  return (
    <>
      {/* 过滤器卡片 */}
      <Card className="backdrop-blur-xl bg-blue-500/10 border-2 rounded-xl overflow-hidden p-4 mb-4" style={{
        ...
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            <ViewFilter
              filters={[
                { key: 'process-code', name: 'process-code' },
                { key: 'process-name', name: 'process-name' },
                { key: 'standard-type', name: 'standard-type' },
              ]}
              classNames={{
      ...
      >
        <TableColumn name="process-code" title="工序编码" width={120} />
        <TableColumn name="process-name" title="工序名称" width={150} />
        <TableColumn name="standard-type" title="标准类型" width={100} />
        <TableColumn name="standard-time" title="标准工时(min)" width={130} />
        <TableColumn name="equipment-type" title="设备类型" width={120} />
        <TableColumn name="status" title="状态" width={100}>
          {(props) => {
            const value = props.value
            const colorMap: Record<string, string> = {
              '启用': 'bg-green-500/20 text-green-400',
              '停用': 'bg-gray-500/20 text-gray-400',
            }
            return (
...
export function LaborStandardPage() {
  return (
    <div className="space-y-0">
      <ViewModel tableId={tableId} initQuery={true}>
          <LaborStandardContent />
        </ViewModel>
    </div>
  )
}

export default LaborStandardPage
