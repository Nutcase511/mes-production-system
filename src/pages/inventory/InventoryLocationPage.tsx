...
const InventoryLocationContent: React.FC = () => {
  return (
    <>
      {/* 过滤器卡片 */}
          ...
          <div className="flex-1">
            <ViewFilter
              filters={[
                ...
                { key: 'location-code', name: 'location-code' },
                { key: 'location-type', name: 'location-type' },
              ]}
              classNames={{
      ...
      >
        <TableColumn name="location-code" title="库位编码" width={120} />
        <TableColumn name="location-name" title="库位名称" width={150} />
        <TableColumn name="warehouse" title="所属仓库" width={120} />
        <TableColumn name="location-type" title="库位类型" width={100} />
        <TableColumn name="zone" title="区域" width={100} />
        <TableColumn name="capacity" title="容量" width={100} />
        <TableColumn name="current-qty" title="当前数量" width={100} />
        <TableColumn name="status" title="状态" width={100}>
          {(props) => {
            const value = props.value
            const colorMap: Record<string, string> = {
              '可用': 'bg-green-500/20 text-green-400',
              '已满': 'bg-red-500/20 text-red-400',
              '维护中': 'bg-yellow-500/20 text-yellow-400',
              '禁用': 'bg-gray-500/20 text-gray-400',
            }
            return (
