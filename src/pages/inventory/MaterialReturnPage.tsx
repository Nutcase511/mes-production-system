    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-blue-200 mt-1">物资退库缴送单管理</p>
        </div>
      </div>
      ...
      <Card className="backdrop-blur-xl bg-blue-500/10 border-2 rounded-xl overflow-hidden"
        ...
        <CardHeader>
          <CardTitle className="text-blue-100">退库记录 ({records.length})</CardTitle>
        </CardHeader>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-blue-400/30">
                    <th className="text-left py-2 px-3 text-blue-200 font-medium">退库单号</th>
                    <th className="text-left py-2 px-3 text-blue-200 font-medium">退库日期</th>
                    ...
                    <th className="text-right py-2 px-3 text-blue-200 font-medium">退库数量</th>
                    <th className="text-left py-2 px-3 text-blue-200 font-medium">退库原因</th>
                    <th className="text-left py-2 px-3 text-blue-200 font-medium">退库人</th>
                ...
                <tbody>
                  {records.slice(0, 20).map((record, index) => (
                    <tr key={record.id || index} className="border-b border-blue-400/20 hover:bg-blue-500/10">
                      ...
                      <td className="py-2 px-3 text-blue-100">{record.return_date || '-'}</td>
                      <td className="py-2 px-3 text-white">{record.material_name || '-'}</td>
                      <td className="py-2 px-3 text-blue-100">{record.spec || '-'}</td>
                      ...
                      <td className="py-2 px-3 text-white">{record.return_by || '-'}</td>
                      <td className="py-2 px-3">
                        <Badge className="bg-blue-500 text-white">{record.status || '待审批'}</Badge>
                      </td>
                    </tr>
                  ))}
                </tbody>
