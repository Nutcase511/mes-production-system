...
const WarehouseTransferContent: React.FC = () => {
  return (
    <>
      {/* 过滤器卡片 */}
      <Card className="backdrop-blur-xl bg-blue-500/10 border-2 rounded-xl overflow-hidden p-4 mb-4" style={{
        ...
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            <ViewFilter
              filters={[
                { key: 'transfer-no', name: 'transfer-no' },
                { key: 'from-warehouse', name: 'from-warehouse' },
                { key: 'to-warehouse', name: 'to-warehouse' },
              ]}
              classNames={{
      ...
      >
        <TableColumn name="transfer-no" title="调拨单号" width={150} />
        <TableColumn name="material-code" title="物料编码" width={120} />
        <TableColumn name="material-name" title="物料名称" width={180} />
        <TableColumn name="from-warehouse" title="源仓库" width={120} />
        <TableColumn name="from-location" title="源库位" width={120} />
        <TableColumn name="to-warehouse" title="目标仓库" width={120} />
        <TableColumn name="to-location" title="目标库位" width={120} />
        <TableColumn name="transfer-qty" title="调拨数量" width={100} />
        <TableColumn name="transfer-date" title="调拨日期" width={120} />
        <TableColumn name="transfer-status" title="状态" width={100}>
          {(props) => {
