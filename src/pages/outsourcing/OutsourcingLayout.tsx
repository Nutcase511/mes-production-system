...
export function OutsourcingLayout() {
  const location = useLocation()
  const { user } = useAuth()
  const showBreadcrumb = user?.role === 'admin' || user?.isSuper === true

  const menuItems = [
    { path: '/outsourcing/list', label: '外协单管理' },
    { path: '/outsourcing/pending', label: '待验收' },
    { path: '/outsourcing/delivery', label: '交货管理' },
    { path: '/outsourcing/quality-check', label: '外协质检' },
    { path: '/outsourcing/certificate', label: '合格证' },
    { path: '/outsourcing/outsourced-deviation', label: '外协超差处理(新)' },
  ]
