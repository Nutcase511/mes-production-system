...
function DataGridTableBase({ children }: { children: ReactNode }) {
  ...
  const { left: leftCols, center: centerCols, right: rightCols } = getPinnedGroups(visibleColumns)
  ...
  return (
    ...
    >
      <colgroup>
        {[...leftCols, ...centerCols].map((column) => (
          <col
            key={column.id}
            style={
              props.tableLayout?.columnsResizable
...
function DataGridTableHeadRow<TData>({
  ...
  const hasRightPinned = headerGroup.headers.some(
  ...
  return (
    ...
    >
      {children}
      {!hasRightPinned && <DataGridTableFillHeadCell />}
    </tr>
  )
}
  ...
  return (
    <th
      ...
      className={cn(
        ...
        props.tableLayout?.columnsPinnable &&
          column.getCanPin() &&
          "[&[data-pinned][data-last-col]]:border-border data-pinned:bg-[hsl(221,35%,12%)] [&:not([data-pinned]):has(+[data-pinned])_div.cursor-col-resize:last-child]:opacity-0 [&[data-last-col=left]_div.cursor-col-resize:last-child]:opacity-0 [&[data-pinned=left][data-last-col=left]]:border-e! [&[data-pinned=right]:last-child_div.cursor-col-resize:last-child]:opacity-0 [&[data-pinned=right][data-last-col=right]]:border-s!",
  ...
  return (
    <td
      ...
      className={cn(
        ...
        props.tableLayout?.columnsPinnable &&
          column.getCanPin() &&
          '[&[data-pinned][data-last-col]]:border-border data-pinned:bg-[hsl(221,35%,12%)] [&[data-pinned=left][data-last-col=left]]:border-e! [&[data-pinned=right][data-last-col=right]]:border-s!',
        column.getIndex() === 0 ||
...
function DataGridTableBodyRow<TData>({
  ...
  const hasRightPinned = row.getVisibleCells().some(
  ...
  return (
    ...
    >
      {children}
      {!hasRightPinned && <DataGridTableFillBodyCell />}
    </tr>
  )
}
...
function DataGridTableRenderedRow<TData>({
  ...
  const { left: leftCells, center: centerCells, right: rightCells } =
    getPinnedGroups(visibleCells)
  ...
  return (
    <Fragment>
      <DataGridTableBodyRow
        ...
        rowRef={rowRef}
      >
        {[...leftCells, ...centerCells].map((cell: Cell<TData, unknown>) => (
          <DataGridTableBodyRowCell cell={cell} key={cell.id}>
            {flexRender(cell.column.columnDef.cell, cell.getContext())}
          </DataGridTableBodyRowCell>
        ))}
        {rightCells.length > 0 && <DataGridTableFillBodyCell />}
        {rightCells.map((cell: Cell<TData, unknown>) => (
          <DataGridTableBodyRowCell cell={cell} key={cell.id}>
            {flexRender(cell.column.columnDef.cell, cell.getContext())}
          </DataGridTableBodyRowCell>
        ))}
        ...
        <DataGridTableHead>
          {table
            ...
            .map((headerGroup: HeaderGroup<TData>, index) => {
              const { left: leftHeaders, center: centerHeaders, right: rightHeaders } =
                getPinnedGroups(headerGroup.headers)

              return (
                <DataGridTableHeadRow headerGroup={headerGroup} key={index}>
                  {[...leftHeaders, ...centerHeaders].map((header, index) => {
                    const { column } = header
                  ...
                  {rightHeaders.length > 0 && <DataGridTableFillHeadCell />}
